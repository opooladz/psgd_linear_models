...
sqrtSigma = np.eye(d) # Identity matrix 
...
# initilize liear layer to be normal_ using pytorch init.
...
sns.lineplot(x=np.arange(len(psgd_mse)), y=psgd_mse, ax=ax, label="PSGD Train MSE")
sns.lineplot(x=np.arange(len(sgd_mse)), y=sgd_mse, ax=ax, label="SGD Train MSE")
sns.lineplot(x=np.arange(len(psgd_mse_test)), y=psgd_mse_test, ax=ax, label="PSGD Test MSE", linestyle='--')
sns.lineplot(x=np.arange(len(sgd_mse_test)), y=sgd_mse_test, ax=ax, label="SGD Test MSE", linestyle='--')
...
plt.tight_layout()
plt.savefig("mse_comparison_log_isotropic.png")
plt.show()
