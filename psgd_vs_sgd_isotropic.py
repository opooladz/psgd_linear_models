...
def empirical_generalization_error(d, mu_1, mu_2, num, w):
    error = 0
...
for epoch in range(num_epochs):
    ...
    if epoch % 100 == 0:
      ...
      num = 10 * d
      ge = empirical_generalization_error(d, mu_1, mu_2, num, w_np)
...
model = linearRegression(d)
model = model.cuda()
optimizer = psgd.Newton(model.parameters(), lr_params=1.5,lr_preconditioner=.5,preconditioner_update_probability=1,grad_clip_max_norm=1)
...
plt.tight_layout()
plt.savefig("psgd_sgd_isotropic_comparison.png")
plt.show()
